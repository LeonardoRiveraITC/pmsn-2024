...
class _DespensaScreenState extends State<DespensaScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder(
        valueListenable: AppValueNotifier.flagProducts,
        builder: (context,value,_) {
          return FutureBuilder(
            future: productsDB!.read("productos"),
  ...
  showModal(context, ProductsModel? producto){
    ...
    if( producto != null ){
      conNombre.text = producto.producto!;
    ...
    final btnAgregar = ElevatedButton.icon(
      onPressed: (){
        ...
        if( producto == null ){
          productsDB!.insert("productos",{
            "producto" : conNombre.text,
            "cantidad" : int.parse(conCantidad.text),
            "caducidad" : conFecha.text
          }).then((value){
            Navigator.pop(context);
            String msj = "";
            if(value>0){
              AppValueNotifier.flagProducts.value = !AppValueNotifier.flagProducts.value;
